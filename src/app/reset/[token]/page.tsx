...
async function resetPassword(formData: FormData) {
  ...
  if (pw !== pw2) return redirect(`/reset/${token}?error=match`);
  const err = validatePasswordStrength(pw);
  if (err) return redirect(`/reset/${token}?error=${encodeURIComponent(err)}`);
  const t = await consumePasswordToken(token);
  if (!t) return redirect(`/reset/${token}?error=invalid`);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-6">
        <div className="bg-white border rounded-2xl p-8 max-w-md">
          <h1 className="text-xl font-semibold mb-2">Bağlantı geçersiz</h1>
          <p className="text-sm text-slate-500">Sıfırlama bağlantısı geçersiz veya süresi dolmuş.</p>
        </div>
      </div>
    );
  }
  return (
    <div className="min-h-screen flex items-center justify-center p-6">
      <div className="bg-white border rounded-2xl p-8 max-w-md w-full">
        <h1 className="text-xl font-semibold mb-1">Şifre sıfırla</h1>
        <form action={resetPassword} className="space-y-3 text-sm mt-3">
          ...
          <button className="w-full bg-slate-900 text-white rounded-lg py-2.5">Şifreyi güncelle</button>
        </form>
      </div>
    </div>
  );
