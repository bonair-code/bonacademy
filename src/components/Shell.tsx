...
export type TrainingStep = {
  label: string;
  sub?: string;
  // "done" — tamamlanmış (yeşil check)
  // "current" — şu an buradayız (teal highlight)
  // "ready" — açıldı ama henüz girilmedi (beyaz, teal kenarlık, tıklanabilir)
  // "locked" — henüz açılmadı (gri, kilit ikonu, tıklanamaz)
            ...
            <div className="mb-3">
              ...
              <ol className="space-y-1 px-1">
                {trainingSteps.map((s, i) => {
                  const last = i === trainingSteps.length - 1;
                  const circleCls =
                    s.state === "done"
                  ...
                  const inner = (
                    <div className="flex items-start gap-3 py-1.5">
                      <div className="flex flex-col items-center shrink-0">
                        <span
                          className={`h-6 w-6 rounded-full border-2 flex items-center justify-center text-[11px] font-semibold ${circleCls}`}
                        >
                          {s.state === "done" ? (
                            <svg
                              viewBox="0 0 24 24"
                      ...
                      <div className="min-w-0 pb-2">
                        <div className={`text-sm leading-tight ${textCls}`}>
                          {s.label}
                        </div>
                        {s.sub && (
                  ...
                  if (s.href && s.state !== "locked") {
                    return (
                      <li key={i}>
                        <Link
                          href={s.href}
                          className="block rounded-lg hover:bg-slate-50 px-2"
                        >
                          {inner}
                        </Link>
                      </li>
